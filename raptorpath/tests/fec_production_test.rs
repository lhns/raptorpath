...
//!
//! These tests verify that the FEC encoding/decoding pipeline handles every
...
/// Encode data and return (source_symbols, repair_symbols, params).
fn encode_block(
    backend: FecBackend,
    data: &[u8],
    symbol_size: u16,
...
fn decode_symbols(
    ...
    let mut decoder = backend.create_decoder(params, transfer_length);
    let mut result = None;
    for sym in symbols {
...
/// All backends to test, with their names and appropriate repair counts.
...
fn test_k_minus_1_source_symbols_cannot_decode() {
    for (name, backend, _) in all_backends() {
        let data = make_data(4096);
        let symbol_size = 512u16;
        let repair_count = 5;
        let (source, _repair, params) = encode_block(backend, &data, symbol_size, repair_count, 0);
        ...
        for sym in source.iter().take(k - 1) {
            ...
            assert!(
                result.is_none(),
                "[{name}] decoder should not produce output with only K-1 source symbols"
            );
        }
        assert!(!decoder.is_decoded(), "[{name}] should not be decoded");
    }
}
...
fn test_exactly_k_source_symbols_decodes() {
    for (name, backend, _) in all_backends() {
        ...
        assert!(decoder.is_decoded(), "[{name}] decoder must be decoded after all K source symbols");
        let decoded = decoded.unwrap_or_else(|| panic!("[{name}] must have decoded data"));
...
fn test_boundary_sweep_k_to_k_plus_r() {
    for (name, backend, _) in all_backends() {
        ...
        let repair_count = 10;
        let (source, repair, params) = encode_block(backend, &data, symbol_size, repair_count, 0);
        ...
        for count in k..=(k + r) {
            ...
            let result = decode_symbols(backend, &feed, params, data.len() as u64);
            assert!(
                result.is_some(),
                "[{name}] failed to decode with {} symbols (K={}, R={})",
                count, k, r
            );
...
/// 4. Drop source symbol 0 (first), add remaining source + repair. Must recover.
#[test]
fn test_drop_first_source_recover_with_repair() {
    for (name, backend, repair_count) in all_backends() {
        let data = make_data(4096);
        let symbol_size = 512u16;
        let (source, repair, params) = encode_block(backend, &data, symbol_size, repair_count, 0);
        ...
        let result = decode_symbols(backend, &symbols, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| panic!("[{name}] must decode after dropping 1 source + adding repair"));
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_drop_last_source_recover_with_repair() {
    for (name, backend, repair_count) in all_backends() {
        ...
        let result = decode_symbols(backend, &symbols, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| panic!("[{name}] must decode after dropping last source + adding repair"));
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_burst_loss_first_half() {
    for (name, backend, repair_count) in all_backends() {
        ...
        let result = decode_symbols(backend, &symbols, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| panic!("[{name}] must decode after first-half burst loss with repair"));
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_burst_loss_second_half() {
    for (name, backend, repair_count) in all_backends() {
        ...
        let result = decode_symbols(backend, &symbols, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| panic!("[{name}] must decode after second-half burst loss with repair"));
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_interleaved_blocks() {
    for (name, backend, _) in all_backends() {
        ...
        let (source0, _repair0, params0) = encode_block(backend, &data0, symbol_size, repair_count, 0);
        let (source1, _repair1, params1) = encode_block(backend, &data1, symbol_size, repair_count, 1);

        let max_len = source0.len().max(source1.len());
        let mut decoder0 = backend.create_decoder(params0, data0.len() as u64);
        let mut decoder1 = backend.create_decoder(params1, data1.len() as u64);
        ...
        assert!(decoder0.is_decoded(), "[{name}] block 0 must decode");
        assert!(decoder1.is_decoded(), "[{name}] block 1 must decode");
...
fn test_symbol_reordering() {
    for (name, backend, _) in all_backends() {
        ...
        let result = decode_symbols(backend, &all_symbols, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| panic!("[{name}] must decode after shuffling symbols"));
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_repair_only_minimum_symbols() {
    for (name, backend, _) in all_backends() {
        ...
        let (_source, repair, params) = encode_block(backend, &data, symbol_size, repair_count, 0);

        let result = decode_symbols(backend, &repair, params, data.len() as u64);
        let decoded = result.unwrap_or_else(|| {
            panic!("[{name}] must decode from repair symbols only (feeding K+10 repair for K source symbols)")
        });
        assert_data_eq(&decoded, &data);
    }
}
...
fn test_data_integrity_varied_sizes() {
    ...
    for (name, backend, _) in all_backends() {
        for &size in &sizes {
            ...
            let result = decode_symbols(backend, &source, params, data.len() as u64);
            let decoded = result.unwrap_or_else(|| {
                panic!("[{name}] must decode data of size {} with all source symbols", size)
            });
            assert_data_eq(&decoded, &data);
        }
    }
...
fn test_massive_loss_insufficient_repair() {
    for (name, backend, _) in all_backends() {
        ...
        assert!(
            total < k,
            "[{name}] total symbols ({total}) must be fewer than K ({k}) for this test"
        );
        ...
        assert!(
            !decoded,
            "[{name}] decoder should NOT succeed with only {total} of {k} required symbols"
        );
