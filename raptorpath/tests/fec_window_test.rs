...
use raptorpath::fec::{
    MettleWindowDecoder, MettleWindowEncoder, RlcWindowDecoder, RlcWindowEncoder,
...
// ===========================================================================
// METTLE window integration tests
// ===========================================================================

fn mettle_config() -> mettle::MettleConfig {
    mettle::MettleConfig::small_window()
}

/// METTLE pipeline helper: frame → encode → simulate loss → decode → extract.
/// Uses extra repair symbols since METTLE's graph structure may need more than
/// one repair per loss for peeling to succeed.
fn mettle_window_pipeline(
    packets: &[Vec<u8>],
    loss_indices: &[usize],
    repair_count: usize,
) -> Vec<Vec<u8>> {
    let mut encoder = MettleWindowEncoder::new(mettle_config(), SYMBOL_SIZE, 42);
...
#[test]
fn test_mettle_window_e2e_no_loss() {
    let packets = make_packets(20, 32);
    let recovered = mettle_window_pipeline(&packets, &[], 0);
    assert_eq!(recovered, packets);
}

#[test]
fn test_mettle_window_e2e_single_loss() {
    let packets = make_packets(10, 32);
    // METTLE may need more repair symbols due to graph structure
    let recovered = mettle_window_pipeline(&packets, &[3], 20);
    assert!(
        recovered.len() >= 10,
        "Expected full recovery with excess repair, got {}/10",
        recovered.len()
    );
}

#[test]
fn test_mettle_window_e2e_multi_loss() {
    let packets = make_packets(10, 32);
    let recovered = mettle_window_pipeline(&packets, &[2, 7], 30);
    // With generous repair, should recover both
    assert!(
        recovered.len() >= 9,
        "Expected at least 9/10 recovered with 30 repair, got {}",
        recovered.len()
    );
}

#[test]
fn test_mettle_window_e2e_framing_roundtrip() {
    let mut encoder = MettleWindowEncoder::new(mettle_config(), SYMBOL_SIZE, 42);
    let mut decoder = MettleWindowDecoder::new(SYMBOL_SIZE);

    let test_data = b"METTLE window pipeline test";
    let framed = frame_window_packet(test_data, SYMBOL_SIZE);
    let sym = encoder.add_source(&framed);
    let results = decoder.add_symbol(&sym);
...
#[test]
fn test_mettle_window_e2e_large_stream() {
    let mut encoder = MettleWindowEncoder::new(mettle_config(), SYMBOL_SIZE, 42);
    let mut decoder = MettleWindowDecoder::new(SYMBOL_SIZE);
    ...
    for (i, pkt) in packets.iter().enumerate() {
        ...
        // Generate repair every 3 source symbols (higher rate for METTLE)
        if source_count % 3 == 0 {
            let repair = encoder.generate_repair();
            let results = decoder.add_symbol(&repair);
            for (_, data) in results {
        ...
        if i > 0 && i % 40 == 0 {
            let advance_to = (i as u64).saturating_sub(20);
            encoder.advance(advance_to);
            decoder.advance(advance_to);
        }
    }
    ...
    assert!(
        recovery_rate > 0.85,
        "Expected >85% recovery with 10% loss and ~33% repair, got {:.1}% ({}/{})",
        recovery_rate * 100.0,
        recovered_count,
        total_packets
    );
...
#[test]
fn test_mettle_window_e2e_data_integrity() {
    let packets: Vec<Vec<u8>> = (1..=10)
        .map(|i| {
            let size = (i * 8).min(SYMBOL_SIZE as usize - 2);
            (0..size).map(|j| (j as u8).wrapping_add(i as u8)).collect()
        })
        .collect();
    ...
    assert_eq!(recovered.len(), packets.len());
    for (i, (got, expected)) in recovered.iter().zip(packets.iter()).enumerate() {
        assert_eq!(got, expected, "METTLE data mismatch at packet {i}");
    }
}
