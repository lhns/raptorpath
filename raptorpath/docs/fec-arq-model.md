...
4. [Unified Correction Symbol Model](#4-unified-correction-symbol-model)
   - [4.1 Why Unify FEC and ARQ?](#41-why-unify-fec-and-arq)
   - [4.2 Correction Symbols — The Unified Concept](#42-correction-symbols--the-unified-concept)
   - [4.3 Three-Stream View: Source, Repair, Retransmit](#43-three-stream-view-source-repair-retransmit)
   - [4.4 Per-Slot Decision](#44-per-slot-decision)
5. [Protocol Mechanics](#5-protocol-mechanics)
   - [5.1 The Retransmit Buffer](#51-the-retransmit-buffer)
   - [5.2 SACK-Extended ACK](#52-sack-extended-ack)
   - [5.3 Per-Symbol Delivery Outcomes](#53-per-symbol-delivery-outcomes)
   - [5.4 The Triangle in Action](#54-the-triangle-in-action)
   - [5.5 Four-Mechanism Composition](#55-four-mechanism-composition)
6. [The Taper Function](#6-the-taper-function)
   - [6.1 Definition](#61-definition)
   - [6.2 Why Match the Loss Distribution?](#62-why-match-the-loss-distribution)
   - [6.3 Total Correction Rate](#63-total-correction-rate)
   - [6.4 The Taper Never Reaches Zero](#64-the-taper-never-reaches-zero)
   - [6.5 Real-Time Adaptation](#65-real-time-adaptation)
...
controls which type. See Section 4.2.
...
distribution: τ(t) = A × (1-q)^t for Gilbert-Elliott channels. See Section 6.
...
sender know exactly which symbols arrived despite gaps. See Section 5.1.
...
continue until ACK, the infinite-tailed taper from Section 6).
...
directly gives us the optimal taper function shape (Section 6).
...
## 4. Unified Correction Symbol Model

### 4.1 Why Unify FEC and ARQ?
...
### 4.2 Correction Symbols — The Unified Concept
...
The taper function (Section 6) determines the **density** of correction
symbols over time. This section explains what happens in each slot.

### 4.3 Three-Stream View: Source, Repair, Retransmit
...
```
  ...
  where r        = taper correction ratio (Section 6)
...
### 5.3 Per-Symbol Delivery Outcomes
...
### 6.2 Why Match the Loss Distribution?
...
### 6.4 The Taper Never Reaches Zero
...
For T_cut = ∞ (ρ = 100%): reduces to r = A/q (Section 6.3).
...
For ρ = 100%: T_cut = ∞ (no finite solution — see Section 5.4).
...
1. **Finite window truncation (resolved):** The taper is infinite-tailed but
   ...
   longer possible — but the retransmit buffer (Section 4.3) still holds the
...
7. **P_arq never defined (resolved).** (Section 5.3, 8.6, Appendix A) [MEDIUM]
   Derived from reliability target:
   P_arq = 1 - (1-rho)/(e x (1-P_fec)). See Section 5.3.
...
14. **Retransmit buffer saturation (resolved).** (Section 5.1) [LOW]
    Resolved: dual-mechanism model — T_cut age eviction + buffer_max size
    backpressure. Buffer is NOT bounded by encoder window. See Section 5.1.

15. **SACK timing and format (resolved).** (Section 5.2) [MEDIUM]
...
24. **BOCD "5-15 batches" — what's a batch (resolved)?** (Section 6.5) [LOW]
