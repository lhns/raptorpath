#!/bin/bash
# THE VERDICT BATTERY — goal #101 ITEM 4, RUN AS SPECIFIED.
# THE SHIPPED CLAMP vs THE STRONGEST MEASURED CHALLENGER THIS PROGRAM PRODUCED.
#
#   sudo bash verdict_battery.sh <seed> [reps]
#
# ITEM 4 asked for the shipped 25/100 ms constants to be put against a derived
# winner. No derived winner exists — the (q, refresh) sweep returned FLAT and
# item 4 closed DOES-NOT-RUN-BY-ITS-OWN-CONDITION twice. THIS BATTERY RUNS IT
# ANYWAY, against the strongest MEASURED challenger anywhere in the record
# rather than against a derived one, so that "survives on merit" is a verdict
# a battery returned and not a region nobody entered.
#
# THE CHALLENGER IS FROM THE RECORD, NOT INVENTED. It is c7-R2Q86's
# configuration — the ONE arm in 140 invocations that realized a hold BELOW its
# cell's self-heal median (hd_p50 = 27 648 us = 0.90x the 30 700 us median) and
# read goodput IN BAND while doing it: fa fraction -19.0%, fa count -17.4%,
# queue p50 -16.9%, queue p99 -13.7%, goodput -4.6%, n = 8. It is instantiated
# PER CELL at that cell's own (q, refresh) point under 16.78.3's grid law, not
# copied as a constant.
#
#     ARM CTL     RWM_HOLDDOWN_Q absent, RWM_REFRESH_FLOOR_US absent
#                 => BYTE-IDENTICAL to the shipped machine (16.78.0)
#     ARM R2Q86   q = 0.864, F = the cell's p50/2 point
#
# TWO ARMS ONLY, INNERMOST, PAIRED WITHIN REP. Every contrast is taken against
# its own rep's control; rows from different reps are never differenced.
#
# THIS SCRIPT EMITS COUNTS. It applies no bar, names no winner, and scores
# nothing. The bars live in the goal-gate pre-registration "THE VERDICT BATTERY
# — PRE-REGISTRATION", which is committed BEFORE this battery is launched, and
# in paper 16.79, which is the decision this battery is pointed at.
#
# ---------------------------------------------------------------------------
# What follows is the (q, refresh) sweep's own header, unedited, because this
# battery reuses that sweep's gate, grid law and arm tables verbatim and the
# provenance of each belongs with the code that reads it.
# ---------------------------------------------------------------------------
#
...
QR_CELLS="${RWM_VRD_CELLS:-c1 c7 sc2 c8}"
QR_ARMS="${RWM_VRD_ARMS:-CTL R2Q86}"
TAG="${RWM_VRD_TAG:-vrd}"
  ...
  case "$2" in
    ...
    R2Q99|R2Q86)
      case "$1" in
        ...
        c7)  echo 3838 ;;
        sc2) echo 12288 ;;
        # c8: orig p50 = 98.3 ms (succ-arrival, 3.20x rep dispersion -- see
        # cell_spec). UPPER rail is PREDICTED to bind (2*srtt >= 100 ms at a
        # c2/c3 dual), so cadence = 4F and F = 49152/4. THE PREDICTION IS
        # CHECKED, NOT ASSUMED: the CADENCE witness reads what is delivered
        # and the pre-registration's F1-c8 clause scores it.
        c8)  echo 12288 ;;
        *)   echo "" ;;
      esac ;;
    R4Q99|R4Q86)
...
cell_shipped_floor() { case "$1" in c1) echo 25000 ;; c7) echo 100000 ;; sc2) echo 100000 ;; c8) echo 100000 ;; *) echo 25000 ;; esac; }
  ...
  case "$2" in
    ...
    R2Q99|R2Q86)
      case "$1" in
        ...
        c8)  echo 49152 ;;   # UPPER rail PREDICTED to bind: cadence = 4F
        *)   echo 0 ;;
      esac ;;
    R4Q99|R4Q86)
      case "$1" in
...
cell_selfheal_p50_us() { case "$1" in c1) echo 24600 ;; c7) echo 30700 ;; sc2) echo 98300 ;; c8) echo 98300 ;; *) echo 0 ;; esac; }
...
VRD_ARM_GATES="RWM_HOLDDOWN_Q RWM_REFRESH_FLOOR_US"
...
VRD_SUBSTRATE_GATES="RWM_DELTA_CAP RWM_SUM_CAP RWM_W_FORM RWM_ALPHA_OVERRIDE"
VRD_CONTAM_GATES="RWM_QUANTILE_CLOCKS RWM_RACK_CLOCKS RWM_DERIVED_SWEEP RWM_COMPOSED_CAP RWM_THREE_TERM \
...
arm_env() { # cell arm -> "RWM_X=v ..."
  ...
  for g in $VRD_ARM_GATES $VRD_SUBSTRATE_GATES $VRD_CONTAM_GATES; do
    case "$g" in
...
cell_spec() {
  case "$1" in
    ...
    # c8 -- the LOSS-HEAVY WITNESS CELL. Transcribed verbatim from
    # hold_battery.sh. It is IN this battery and NOT in the (q, refresh)
    # sweep, and the pre-registration states in advance what that does and
    # does not license: its [SUCC] orig p50 carries 3.20x rep dispersion, so
    # its cell-scaled (q, refresh) point is derived from an input the
    # successor-arrival pass recorded NOT USABLE. It is a WITNESS row, never
    # a vote.
...
check_and_parse() { # name cell arm alpha cpus cpuc pingp qp
  ...
  python3 ./alpha_parse.py "$cell" "$arm" "$alpha" "$SEED_ARG" "$REP" \
    ...
    >> "$OUT" 2>&1 || echo "VRD-PARSE-FAIL $name rep=$REP" >> "$OUT"
  ...
  for g in $VRD_ARM_GATES $VRD_SUBSTRATE_GATES $VRD_CONTAM_GATES; do
    ...
    case " $VRD_ARM_GATES $VRD_SUBSTRATE_GATES " in
      *" $g "*)
  ...
  echo "VRDWITNESS {\"cell\":\"$cell\",\"arm\":\"$arm\",\"seed\":$SEED_ARG,\"rep\":$REP,\"rc\":$RC,\"mean_mbps\":$mb,\"band\":[$lo,$hi],\"band_applies\":$applies,\"in_band\":$inband,\"lossy\":$lossy,\"q_expect\":\"$q_exp\",\"q_got\":\"$q_got\",\"floor_expect\":\"$floor_exp\",\"floor_got\":\"$floor_got\",\"cadence_want\":$want_cad,\"cadence_p50\":$qp50,\"cadence_min\":$qmin,\"cadence_max\":$qmax,\"cadence_below\":\"$below\",\"cadence_floor_shipped\":$shipped,\"qclk_evals\":$qev,\"qclk_kept\":$qkept,\"hold_law_n\":$hlaw,\"hold_evals\":$hev,\"hold_sup\":$hsup,\"hold_emit\":$hemit,\"hold_t_us\":\"$htus\",\"hold_hd_p50_us\":\"$hd50\",\"hold_n_req\":\"$hn\",\"fcause_n\":$fcn,\"fcause_timer\":$fctimer,\"fcause_gap_data\":$fcgap,\"fcause_gap_refresh\":$fcgapr,\"fcause_other\":$fcother,\"fcause_unattr\":$fcunattr,\"succ_det\":$scdet,\"succ_res\":$scres,\"succ_orig_frac\":\"$scof\",\"selfheal_p50_us\":$(cell_selfheal_p50_us "$cell"),\"rfa_fires\":$rfa_fires,\"rfa_false\":$rfa_false,\"rfa_false_frac\":\"$rfa_ff\",\"fails\":\"${FAILS# }\"}" \
    | tee -a "$WIT" >> "$OUT"
}
...
{
  echo "=== VERDICT BATTERY (goal #101 item 4) seed=$SEED_ARG reps=$REPS $(date -u +%FT%TZ)"
  echo "CONTRACT goal-gate \"THE (q, refresh) SWEEP -- PRE-REGISTRATION\", and nothing else. Paper 16.78 is the derivation."
  echo "CELLS $QR_CELLS   (c1/c7/sc2 are the THREE STABLE cells and carry the verdict; c8 is the LOSS-HEAVY WITNESS cell and carries none -- its [SUCC] orig p50 has 3.20x rep dispersion, so its cell-scaled point is derived from an input the successor-arrival pass recorded NOT USABLE. c8L is not run at all: 52x.)"
  echo "ARMS-MEANING CTL = the shipped machine, both gates ABSENT, byte-identical; R2Q86 = the STRONGEST MEASURED CHALLENGER (c7-R2Q86 s configuration) at each cell s own 16.78.3 grid point"
...
for CELL in $QR_CELLS; do
  for A in $QR_ARMS; do
    ...
    N=$(grep -cF "VRDWITNESS {\"cell\":\"$CELL\",\"arm\":\"$A\"" "$OUT" || true); N="${N:-0}"
    echo "ARMCOUNT $CELL-$A rows=$N/$WANT" >> "$OUT"
    [ "$N" -eq 0 ] && echo "ARM-VANISHED $CELL-$A" >> "$OUT"
  done
done
echo "VERDICT-BATTERY-DONE seed=$SEED_ARG $(date -u +%FT%TZ)" >> "$OUT"
echo VERDICT-BATTERY-DONE-$SEED_ARG
