#!/bin/bash
# THE T-LAG BATTERY — THE DISCIPLINE-16 CALIBRATION, WHICH IS ALSO THE SMOKE.
#
#   sudo nohup bash tlagb_calib.sh >/home/vibe/tlagb/calib.out 2>&1 &
#
...
#     because FOUR OF THE FIVE THINGS IT READS ARE NOT THE SHIPPED CONTROL, AND
#     `tlag_us` HAS NEVER BEEN ON A WIRE AT ALL:
#
#     * `rvar_us` / `qsp_us` / `msd_us` / `tlag_us` — the candidates, and the
#       whole measurement. They must be present WITH `/n` counts on EVERY path
#       entry of EVERY [DIAG] block at BOTH endpoints (`W7`, five tokens). A
#       missing token is not a missing column, it is the measurement failing.
...
#       the t-lag probe functionals are not.
#
# THE DUMP IS OFF HERE, LIKE THE SCORED RUN. The calibration must smoke the
# SAME configuration the scored battery runs, so it inherits the battery's
# `unset RWM_RTT_DUMP` and its W-DUMP assertion. The clause-B pass has its own
# script (`tlagb_bpass.sh`) and its own ledger.
#
# Read, in the calibration ledger, BEFORE launching tlagb_all.sh:
#   ABORT / ABORT-GEN-PLATEAU / SUBSTRATE-FAIL / INSTRUMENT-FAIL-GATE /
#   INSTRUMENT-FAIL-PROBE / W7-FAIL-CLI / TLAGB-DUMP-ON-FAIL /
#   TLAGB-PARSE-FAIL / OUT-OF-BAND
...
#
# WATCHER NOTE: `pgrep -f tlagb_calib.sh` matches the WATCHER'S OWN shell. Watch
...
{
  echo "TLAGB-CALIB start $(date -u +%FT%TZ) load=$(cat /proc/loadavg)"
  echo "TLAGB-CALIB one rep per cell, seed 42 — the discipline-16 headroom pass AND the smoke"
  echo "TLAGB-CALIB NOTHING HERE IS A RESULT (n = 1)"
} > "$OUTDIR/calib-era.txt"
RWM_TLAGB_TAG=tlagb-calib bash tlagb_battery.sh 42 1
echo "TLAGB-CALIB end $(date -u +%FT%TZ) load=$(cat /proc/loadavg)" >> "$OUTDIR/calib-era.txt"
...
LEDGER="$OUTDIR/tlagb-calib-s42.log"
rm -f "$OUTDIR/FAILED-CALIB"
if [ -s "$LEDGER" ] && grep -q "TLAGB-BATTERY-DONE" "$LEDGER"; then
  python3 ./tlagb_report.py "$LEDGER" \
    > "$OUTDIR/tlagb-calib-report.txt" 2>&1 || true
  touch "$OUTDIR/DONE-CALIB"
  echo TLAGB-CALIB-DONE
else
  {
    echo "TLAGB-CALIB FAILED $(date -u +%FT%TZ)"
    echo "  no ledger at $LEDGER, or no TLAGB-BATTERY-DONE line in it."
