#!/bin/bash
# THE T-LAG ESTIMATOR BATTERY — the sigma battery's fifth-gauge successor.
#
#   sudo bash tlagb_battery.sh <seed> [reps]
#
# Contract: goal-gate "THE SIGMA ESTIMATOR — THE BATTERY, PRE-REGISTRATION"
# as carried forward by the t-lag amendment, scored against "THE SIGMA
# ESTIMATOR — THE ACCEPTANCE BAR" and against nothing else.
#
...
# shipped clamp runs exactly as it ships. The FIVE gauges — `sig_us` (shipped,
# its own control) and the four candidates `rvar_us` / `qsp_us` / `msd_us` /
# `tlag_us` — are emitted SIDE BY SIDE by one `format!` in `net/diag.rs`, per
# path, per `[DIAG]` block, from the SAME RTT sample stream, in the SAME run.
# One invocation scores all five. That layout IS the design: every comparison
# is PAIRED and none is across sessions.
#
...
# What replaces it is `W7` — the five fields present WITH their `n` counts on
...
#
# THE RTT DUMP IS OFF HERE AND THAT IS LOAD-BEARING, NOT HOUSEKEEPING. The
# clause-B pass (`tlagb_bpass.sh`) is the ONLY script that sets
# `RWM_RTT_DUMP=1`. The dump writes MEGABYTES of sender stderr, and SENDER-SIDE
# DISPERSION IS THE QUANTITY THIS BATTERY MEASURES — a scored invocation that
# inherited the dump would be measuring its own instrument. `unset` here plus
# the `W-DUMP` [GATES] assertion below make that a HARD ABORT rather than a
# footnote.
#
...
#   W7  all FIVE gauge tokens with /n counts on every path entry, BOTH sites
#       — THIS BATTERY'S OWN, and the reachability gate for the unit under test
#   W-DUMP  [GATES] RWM_RTT_DUMP=0 on both endpoints — a HARD ABORT cause
#
...
#
# WATCHER NOTE: `pgrep -f tlagb_battery.sh` matches the WATCHER'S OWN shell.
# Watch the SENTINEL or the ledger's TLAGB-BATTERY-DONE line — never the
# process table (MEASUREMENT DISCIPLINE 13).
set -uo pipefail
...
SB_CELLS="${RWM_TLAGB_CELLS:-c1 c7 c8 c8L sc2}"
TAG="${RWM_TLAGB_TAG:-tlagb}"
BIN=/home/vibe/raptorpath/target/release/raptorpath
OUT="/home/vibe/tlagb/${TAG}-s${SEED_ARG}.log"
DDIR="/home/vibe/tlagb/diag"
...
# ...and the dump, for the reason in the header: the scored S/C battery MUST
# run with it OFF or it measures its own sender-side instrument.
unset RWM_RTT_DUMP
...
check_and_parse() { # cell rep
  ...
  # ── W-DUMP. THE DUMP MUST BE OFF, AND THIS IS A HARD ABORT CAUSE ─────────
  # `unset RWM_RTT_DUMP` above removes it from THIS process; the [GATES] echo
  # prints the RESOLVED value at the binary, which is the only reading that
  # settles whether the dump was live inside the run that produced these rows.
  # A scored invocation with the dump ON measured its own instrument: megabytes
  # of sender stderr against the sender-side dispersion under test.
  local rd_c rd_s
  rd_c=$(printf '%s' "$gl_c" | grep -o "RWM_RTT_DUMP=[01]")
  rd_s=$(printf '%s' "$gl_s" | grep -o "RWM_RTT_DUMP=[01]")
  echoline="$echoline RWM_RTT_DUMP=${rd_c:-absent}/${rd_s:-absent}(exp0)"
  { [ -n "$rd_c" ] && [ "$rd_c" != "RWM_RTT_DUMP=0" ]; } \
    && echo "TLAGB-DUMP-ON-FAIL $name rep=$REP cli='$rd_c' (dump LIVE on a scored invocation; row VOID)" >> "$OUT"
  { [ -n "$rd_s" ] && [ "$rd_s" != "RWM_RTT_DUMP=0" ]; } \
    && echo "TLAGB-DUMP-ON-FAIL $name rep=$REP srv='$rd_s' (dump LIVE on a scored invocation; row VOID)" >> "$OUT"
  ...
  #    The clause-C1 exclusions are applied by tlagb_report.py against the `n`
  ...
  python3 ./tlagb_parse.py "$cell" "$SEED_ARG" "$REP" "$C" "$S" "$QF" \
      ${PFS[@]+"${PFS[@]}"} \
    >> "$OUT" 2>&1 || echo "TLAGB-PARSE-FAIL $name rep=$REP" >> "$OUT"
  ...
  # own regex is the thing a token change would break first. FIVE tokens now —
  # `tlag_us` sits immediately after `msd_us` on the same path block, so a
  # group count that is not 5x the path count is the emission, not the parse.
  local t f n_c n_s
  for t in sig_us rvar_us qsp_us msd_us tlag_us; do
  ...
  echo "TLAGBBAND {\"cell\":\"$cell\",\"seed\":$SEED_ARG,\"rep\":$REP,\"rc\":$RC,\"mbps\":$mb,\"band\":[$lo,$hi],\"in_band\":$inband,\"gen_plateau\":$plateau,\"lossy\":$lossy}" \
    | tee -a "/home/vibe/tlagb/${TAG}-witness-s${SEED_ARG}.jsonl" >> "$OUT"
}
...
{
  echo "=== TLAGB BATTERY seed=$SEED_ARG reps=$REPS $(date -u +%FT%TZ)"
  echo "CONTRACT goal-gate 'THE SIGMA ESTIMATOR — THE BATTERY, PRE-REGISTRATION' + the t-lag amendment"
  echo "SCORED-AGAINST goal-gate 'THE SIGMA ESTIMATOR — THE ACCEPTANCE BAR' and nothing else"
  echo "CELLS $SB_CELLS"
  echo "GAUGES sig_us rvar_us qsp_us msd_us tlag_us — five, side by side, one format!, one RTT stream"
  echo "ARMS  NONE — the shipped stack, RWM_QUANTILE_CLOCKS OFF, RWM_ALPHA_OVERRIDE ABSENT. No clock is touched."
  echo "DUMP  RWM_RTT_DUMP UNSET AND ASSERTED 0 — the clause-B pass (tlagb_bpass.sh) is the only script that sets it"
...
echo "=== CELLCOUNTS (rows, NOT live n — see tlagb_report.py) $(date -u +%FT%TZ)" >> "$OUT"
for CELL in $SB_CELLS; do
  N=$(grep -c "\"cell\":\"$CELL\"" "$OUT" || true); N="${N:-0}"
  echo "CELLCOUNT $CELL rows=$N/$REPS" >> "$OUT"
  [ "$N" -eq 0 ] && echo "CELL-VANISHED $CELL" >> "$OUT"
done
echo "TLAGB-BATTERY-DONE seed=$SEED_ARG $(date -u +%FT%TZ)" >> "$OUT"
echo TLAGB-BATTERY-DONE-$SEED_ARG
