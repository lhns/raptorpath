#!/bin/bash
# GOAL "HONEST INPUTS" phase 4 — THE FLIP BATTERY (the one that decides the
# default RECOMMENDATIONS; per the standing rule it flips nothing itself).
# Contract: goal-gate "Honest Inputs — FLIP BATTERY — PRE-REGISTRATION"
# (own commit, before any run). That block is scored against, never modified.
#
#   sudo bash flip_battery.sh <seed> [reps] [c1_reps]
#
# SIX ARMS, same-session interleaved per cell (discipline 3):
#
#   A    (unset)                              shipped control
#   H    RWM_HONEST_ANCHOR=1                  F7: the value-identical deque on
#                                             the shipped feed — pure
#                                             no-regression arm
#   DH   RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1   control: must REPRODUCE the −13%
#   DHU  DH + RWM_STORE_CAP_UNIFIED=1         H1's completion (F2)
#   BH   RWM_THREE_TERM=1 RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1 RWM_HONEST_K=1
#                                             the composition as battered
#   BHU  BH + RWM_STORE_CAP_UNIFIED=1         the user-facing candidate (F1)
#
# CELLS (c1 at n=12 — sized to the measured sigma=33.1; the rest n=8):
#   c1    topo c1/c1 single 400 MB   F1/F2/F4 + the F3 mechanism gauge
#   jit25 adv  50 MB                 parity + the [3T] limit in RELATION form
#   sc2   topo c2/c2 single 100 MB   F6: the halved-RTT win must survive
#   c7    topo c2/c2 dual 200 MB     parity floor (DHU/A >= 0.95) + F4
#   c8    topo c2/c3 dual 25 MB      F5's pre-named risk cell; abort class
#
# INSTRUMENTS on every invocation: the `CPU: CPUSRV=/CPUCLI=` gauge (F4/F7 are
# scored on it — absent = INSTRUMENT-FAIL, loud), RWM_DIAG=1 (khr=/kraw=,
# win=occ/cap, wait[], and the `[SF]` saturation-filter gauge — F3's
# per-invocation capture on EVERY arm), tc -s qdisc capture (discipline 16,
# same-session headroom beside every target), RWM_LATPROBE=1 on topo cells.
#
# LIVENESS, asserted per arm and per direction BEFORE any number is read
# (discipline 1/15): all FIVE gates two-sided on the `[GATES]` line of BOTH
# endpoints; the honest-anchor / honest-K / `unified store-cap path set
# ACTIVE` echoes PRESENT exactly on their arms and ABSENT elsewhere;
# `[3T] eng=1` on BH/BHU. An arm whose echo set is wrong is VOID and re-run
# symmetrically, not explained.
#
# ABORT ≠ DNF ≠ INSTRUMENT-FAIL, as encoded in flip_parse.py (no summary at
# all = ABORT; the seed-7 topo-ping abort class is handled by symmetric
...
FLIP_CELLS="${RWM_FLIP_CELLS:-c1 jit25 sc2 c7 c8}"
FLIP_ARMS="${RWM_FLIP_ARMS:-A H DH DHU BH BHU}"
TAG="${RWM_FLIP_TAG:-flip}"
BIN=/home/vibe/raptorpath/target/release/raptorpath
OUT="/home/vibe/flip/${TAG}-s${SEED_ARG}.log"
DDIR="/home/vibe/flip/diag"
mkdir -p "$(dirname "$OUT")" "$DDIR"

arm_env() { case "$1" in
  A)   echo "" ;;
  H)   echo "RWM_HONEST_ANCHOR=1" ;;
  DH)  echo "RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1" ;;
  DHU) echo "RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1 RWM_STORE_CAP_UNIFIED=1" ;;
  BH)  echo "RWM_THREE_TERM=1 RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1 RWM_HONEST_K=1" ;;
  BHU) echo "RWM_THREE_TERM=1 RWM_PLAIN_RS=1 RWM_HONEST_ANCHOR=1 RWM_HONEST_K=1 RWM_STORE_CAP_UNIFIED=1" ;;
esac; }
arm_3t() { case "$1" in BH|BHU) echo 1 ;; *) echo 0 ;; esac; }
arm_rs() { case "$1" in A|H) echo 0 ;; *) echo 1 ;; esac; }
arm_ha() { case "$1" in A) echo 0 ;; *) echo 1 ;; esac; }
arm_hk() { case "$1" in BH|BHU) echo 1 ;; *) echo 0 ;; esac; }
arm_u()  { case "$1" in DHU|BHU) echo 1 ;; *) echo 0 ;; esac; }
...
check_and_parse() {
  ...
  # F4/F7's gauge is the mechanism's conviction gauge: absent = INSTRUMENT-FAIL.
  [ -z "$cpuc" ] && echo "INSTRUMENT-FAIL-CPU $name rep=$REP" >> "$OUT"
  # F3's gauge: the [SF] line must be present on every live invocation.
  local sfn
...
run_adv() { # jit25 arm — adv_cells substrate, tt_adv.sh conventions + CPU gauge
  ...
  sleep 1
  # CPU gauge, both roles (F4/F7's instrument, perf_rwm_c.sh's exact method):
...
run_one() { # cell arm
  case " $FLIP_CELLS " in *" $1 "*) ;; *) return 0 ;; esac
  case " $FLIP_ARMS " in *" $2 "*) ;; *) return 0 ;; esac
  # c1 runs to C1REPS (n=12, sized to sigma=33.1); the rest stop at REPS.
...
{
  echo "=== FLIP BATTERY seed=$SEED_ARG reps=$REPS c1reps=$C1REPS cells='$FLIP_CELLS' arms='$FLIP_ARMS' $(date -u +%FT%TZ)"
...
for REP in $(seq 1 "$REPMAX"); do
  for CELL in $FLIP_CELLS; do
    for ARM in $FLIP_ARMS; do
      run_one "$CELL" "$ARM"
    done
  done
done
