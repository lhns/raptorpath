#!/usr/bin/env python3
"""Per-invocation parser for the HONEST-INPUTS FLIP battery (goal-gate
"Honest Inputs — FLIP BATTERY — PRE-REGISTRATION" — the CONTRACT; nothing
here may reinterpret it).

A SEPARATE parser from `hi_parse.py` on purpose: that is the instrument the
phase-2 verdicts were read off and it stays byte-identical. This one carries
the same abort rule (no summary at all = ABORT, not DNF), the same gauges,
plus what the flip battery is scored on and phase 2 was not:

  FIFTH GATE   — `[GATES] RWM_STORE_CAP_UNIFIED` two-sided on both endpoints
                 plus the `unified store-cap path set ACTIVE` echo, counted
                 on both logs. An arm without its verified echo set is VOID
                 (re-run symmetrically, not explained).
  [SF] GAUGE   — the saturation-filter population (F3), cumulative last
                 sample of the client log: ticks / live_sum / active_sum /
                 short_ticks / zero_ticks. zero/ticks is the fraction of
                 dyn-cap refresh instants where `active_paths()` was EMPTY.
  CAP-AT-BOOT  — the CONSUMED-cliff gauge (F3's second prong): the fraction
                 of steady [DIAG] samples whose effective store cap read
                 <= store_boot_cap (128), i.e. the cap having fallen out of
                 its own law. (The raw [SF] gauge is downstream-coupled to
                 its consumer — goal-gate "Store-Cap Triplication" P0 point
                 4 — so the U-arms' cliff REMOVAL is read here, never off
                 zero_ticks.)

Everything else (goodput, [3T] readout, khr/kraw, occupancy, ping probe, tc
utilisation, wait attribution, eviction) keeps hi_parse.py's definitions so
numbers pool across sessions without a second dialect.

usage: flip_parse.py <cell> <arm> <seed> <rep> <client.log> <server.log> \
...
# ── liveness: [GATES] resolved values, both endpoints, all FIVE gates ────
def gate(lines, name):
...
gates = {
    ...
    "active_u_cli": sum(1 for l in cli if "unified store-cap path set ACTIVE" in l),
    "active_u_srv": sum(1 for l in srv if "unified store-cap path set ACTIVE" in l),
}

# ── the [SF] saturation-filter gauge (F3), cumulative: last line wins ────
sf_re = re.compile(
...
# ── the law's own readout: [3T] cap/window/slack (tt_parse dialect) ──────
tt_re = re.compile(
...
# F3's second prong: steady DIAG samples with the cap fallen to <= boot (128).
capboot = {
    "capboot_n": len(occap),
...
out = {"cell": cell, "arm": arm, "seed": int(seed), "rep": int(rep),
       ...
       # the law readout + the RELATION-form inputs (jit25)
       "tt_eng1": tt_eng, "cap_med": med(caps), "window_med": med(wins),
       "slack_med": med(slacks), "span_med": med(spans),
       "swratio_med": med(swr),
       # the K decomposition + in-cell RTprop (jit25's relation gauges)
...
out.update(dgq_out)
out.update(tc)
print("FLIPRESULT " + json.dumps(out))
